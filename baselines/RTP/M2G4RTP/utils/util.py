...
def train_val_test(train_loader, val_loader, test_loader, model, device, process_batch, test_model, params, save2file):
    ...
    model_name = model.model_file_name() + f'{time.time()}' + params['model'] + str(params['hidden_size']) +"_"+ str(params.get('rl_ratio', None))
    model_path = ws + f'/data/dataset/{params["dataset"]}/sort_model/{model_name}'
    params['model_path'] = model_path
    dir_check(model_path)

    file_name = f'val_new_{params["model"]}_{device.index}.txt'
    ...
    for epoch in range(params['num_epoch']):
        ...
        print('\nval result:', val_result.route_eta_to_str(), 'Best krc:', round(early_stop.best_metric(),4), '| Best epoch:', early_stop.best_epoch)
