...
def subworker(args, flags, T, is_print=False, random_seed=2021, mode='train'):
    ...
    file_name = "xl_"+ str(args.batch_size) + "_" + args.dataset  + ".txt"
    model_path = './logs/'+ args.model +'/'+args.dataset+ f'/seed{random_seed}/checkpoints/'+ T +'_ckpt.pt'
...
def experiment(args, T, mode='train'):
    ...
    flags_path = os.path.join(os.path.dirname(__file__), 'flags.pkl')
