...
class Metric(object):
    ...
    def eta_filter_len(self, prediction, label, label_len, input_len, eta_pred, eta_label):
        """
        filter the input data,  only evalution the data within len_range
        """
        pred_f = []
        ...
        return pred_f, label_f, label_len_f, input_len_f, eta_pred_f, eta_label_f
    ...
    def update_eta(self, prediction, label, label_len, input_len, eta_pred, eta_label) :
        def tensor2lst(x):
            try:
                return x.cpu().numpy().tolist()
            except:
        ...
        prediction, label, label_len, input_len, eta_pred, eta_label = [tensor2lst(x) for x in [prediction, label, label_len, input_len, eta_pred, eta_label]]

        # process the prediction
        prediction, label, label_len, input_len, eta_pred, eta_label = self.eta_filter_len(prediction, label, label_len, input_len, eta_pred, eta_label)
        ...
        krc = np.array([kendall_rank_correlation(pre, lab, lab_len) for pre, lab, lab_len in
    ...
    def eta_to_dict(self) -> Dict:
        ...
        result.update({f'acc_eta@{i}': self.acc_eta[i].avg for i in range(len(self.acc_eta_list))})
        return result
    ...
    def route_eta_to_str(self):
        ...
        return f"krc:{krc} | lsd:{lsd} | ed:{ed} | hr@1:{hr[0]} | hr@2:{hr[1]} | hr@3:{hr[2]} | acc@1:{acc[0]} | acc@2:{acc[1]} | acc@3:{acc[2]} | rmse:{rmse} | mae: {mae} | mape: {mape} | acc_eta@20:{acc_eta[1]}|acc_eta@30:{acc_eta[2]}"
