...
from algorithm.rankepta.dataset import RankEptaDataset, ModelDataset


from algorithm.rankepta.time_predictor import RankEPTA
from algorithm.rankepta.train_route import main as route_main
...
def process_batch_eta(batch, model, device, params):
    ...
    V, V_reach_mask, label, label_len, V_at, sort_idx, start_fea = batch

    eta = model(V, V_reach_mask, sort_idx, start_fea)
...
def get_eta_result(eta_pred, eta_label, route_label_len, label_route):
    ...
    for i in range(B*T):
        if route_label_len[i].long().item() != 0:
            eta_pred_result[i][:eta_label_len[i].long().item()] = eta_pred[i][route_label[i][:eta_label_len[i].long().item()]]
...
def train_val_test(train_loader, val_loader, test_loader, model, device, process_batch, test_model, params, save2file):
    ...
    model_name = model.model_file_name() + f'{time.time()}' + params['model'] + str(params['hidden_size'])
    model_path = ws + f'/data/dataset/{params["dataset"]}/sort_model/{model_name}'
    params['model_path'] = model_path
    dir_check(model_path)
    file_name = f'val_{params["model"]}_{device.index}.txt'
    with open(file_name, 'a') as file:
        file.write("\n\n" + params["dataset"] + "__" + model_name + "\n")
    ...
    for epoch in range(params['num_epoch']):
        ...
        with open(file_name, 'a') as file:
            file.write(f'Epoch {epoch}: {val_result.eta_to_str()} |Best epoch: {early_stop.best_epoch}\n')
    ...
    with open(file_name, 'a') as file:
        file.write(f'Test {epoch}: { test_result.eta_to_str()}\n')
...
def main(params):
    ...
    def get_dataloader(data_list, sort_id_list, is_test=False):
        ...
        for i in range(len(data_list)):
            ...
            start_fea = data_list[i]['start_fea']
            sample_num = len(V)
            data_loader = DataLoader(dataset=ModelDataset(V, V_reach_mask, route_label, label_len, time_label, sort_idx, sample_num, start_fea), batch_size=params['batch_size'], shuffle=False, drop_last=True)
    ...
    train_path = f'./LaDe/dataset/{params["dataset"]}/train.npy'
    val_path = f'./LaDe/dataset/{params["dataset"]}/val.npy'
    test_path = f'./LaDe/dataset/{params["dataset"]}/test.npy'
    ...
    model.to(device)
    from algorithm.rankepta.time_predictor import save2file
