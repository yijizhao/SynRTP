...
def idx_weight(i, mode='linear'):
    ...
    if mode == 'sigmoid': return _sigmoid(5 - i)  # focus on top-5
    if mode == 'no_weight': return 1
    if mode == 'log': return 1 / math.log(2 + i)  # i starts from 0
...
def edit_distance(pred, label):
    ...
    """
    # Requires external `edit_distance` package (pip-install edit-distance).
...
class Metric(object):
    """Collector for multiple evaluation metrics (HR, KRC, LSD, ED, RMSE, etc.).

    Designed for batched route/ETA evaluation; stores averages per metric.
    """
    def __init__(
        self,
        length_range,
    ...
    def to_dict(self) -> Dict:
        ...
        result.update({'lsd': self.lsd.avg, 'lmd': self.lmd.avg, 'krc': self.krc.avg, 'ed': self.ed.avg, 'mae':self.mae.avg})
        return result
    ...
    def to_str(self):
        ...
        return f'krc:{krc} | lsd:{lsd} | ed:{ed} | hr@1:{hr[0]} | hr@2:{hr[1]} | hr@3:{hr[2]} | acc@1:{acc[0]} | acc@2:{acc[1]} | acc@3:{acc[2]} | mae:{mae} | mape:{mape} | rmse:{rmse} | acc_eta@10:{acc_eta[0]}| acc_eta@20:{acc_eta[1]}| acc_eta@30:{acc_eta[2]}| '
    ...
    def route_eta_filter_len(self, prediction, label, label_len, eta_pred, eta_label):
        ...
        """
        # Keep entries whose label length falls within configured `len_range`.
        pred_f = []
        label_f = []
        label_len_f = []
        eta_pred_f = []
