...
def get_workspace_modern():
    workspace_path = Path(__file__).resolve().parent.parent.parent.parent.parent
...
def get_common_params():
    ...
    parser.add_argument('--task', type=str, default='route', help='food_cou or logistics')
    ...
    parser.add_argument('--sort_x_size', type=int, default=8, help='6')
...
def train_val_test(train_loader, val_loader, test_loader, model, device, process_batch, test_model, params, save2file):
    ...
    if model_path != None:
        ...
        with open('tempt.txt', 'a') as file:
            if params['task'] == 'route':
                file.write(f'Test : {test_result.to_str()}\n')
            else:
    ...
    for epoch in range(params['num_epoch']):
        ...
        if params['task'] == 'route':
            ...
            with open(file_name, 'a') as file:
                file.write(f'Epoch {epoch}: {val_result.route_eta_to_str()} |Best epoch: {early_stop.best_epoch}\n')
            if is_best_change:
    ...
    with open(file_name, 'a') as file:
        if params['task'] == 'route':
            file.write(f'Test {epoch}: {test_result.route_eta_to_str()}\n')
        else:
            file.write(f'Test {epoch}: {test_result.route_eta_to_str()}\n')
    return params
...
def get_model_function(model):
    model_dict = {}
    import algorithm.rankepta.route_predictor as ranketpa_route
