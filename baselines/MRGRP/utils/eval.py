...
class Metric(object):
    ...
    def preprocess_for_metric_calculation(self, predictions, labels, label_lens, eta_pred, eta_label):
        ...
        for i in range(len(predictions)):
            ...
            if all_true_labels_found:
                ...
                processed_eta_pred.append(pred_eta)
                processed_eta_label.append(true_eta)
            else:
                deleted_count += 1

        return processed_predictions, processed_labels, processed_label_lens, deleted_count, processed_eta_pred, processed_eta_label




    def update_route(self, results, label_len):
        prediction = results['selections'] 
        label = results['label_selections'] 
        ...
        prediction, label, label_len = self.route_eta_filter_len(prediction, label, label_len)

        prediction, label, label_len, count = self.preprocess_for_metric_calculation(prediction, label, label_len)
        ...
        pred = []
        for p, inp_len in zip(prediction, label_len):
    ...
    def update_route_eta(self, results):
        prediction = results['selections'] 
        label = results['label_selections'] 
        batch_size, graph_size = label.shape
        eta_label = results["label_etr"].float()
        etr_predictions = results["etr"].view(batch_size, graph_size, 9)
        eta_pred = etr_predictions[:, :, 4]
        ...
        prediction, label, eta_pred, eta_label = [
            tensor2lst(x) for x in [prediction, label, eta_pred, eta_label]
        ]
        label_len = (np.array(label) >= 0).sum(axis=1).tolist()


        prediction, label, label_len, eta_pred, eta_label = self.route_eta_filter_len(prediction, label, label_len, eta_pred, eta_label)

        prediction, label, label_len, count, eta_pred, eta_label = self.preprocess_for_metric_calculation(prediction, label, label_len, eta_pred, eta_label)
        self.del_count += count
    ...
    def to_str_route(self):
        hr = [round(x.avg, 4) for x in self.hr]
        acc = [round(x.avg, 4) for x in self.acc]
        krc = round(self.krc.avg, 4)
        lsd = round(self.lsd.avg, 4)
        ed = round(self.ed.avg, 4)
    ...
    def route_eta_to_str(self):
        ...
        lsd = round(self.lsd.avg, 4)
        ed = round(self.ed.avg, 4)
        rmse = round(self.rmse.avg, 4)
        mae = round(self.mae.avg, 4)
        mape = round(self.mape.avg, 4)
        acc_eta = [round(x.avg, 4) for x in self.acc_eta]
        return f"krc:{krc} | lsd:{lsd} | ed:{ed} | hr@1:{hr[0]} | hr@2:{hr[1]} | hr@3:{hr[2]} | acc@1:{acc[0]} | acc@2:{acc[1]} | acc@3:{acc[2]} | rmse:{rmse} | mae: {mae} | mape: {mape} | acc_eta@10:{acc_eta[0]} | acc_eta@20:{acc_eta[1]}|acc_eta@30:{acc_eta[2]}"
